...
export default class ParseApp {
  ...
  async getCustomDomain() {
    try {
      return (
        await axios.get(
          // eslint-disable-next-line no-undef
          `${b4aSettings.BACK4APP_API_PATH}/parse-app/${this.slug}/custom-domain`,
          { withCredentials: true }
        )
      ).data;
    } catch (err) {
  ...
  async getEnvVars() {
    try {
      return (
        await axios.get(
          // eslint-disable-next-line no-undef
          `${b4aSettings.BACK4APP_API_PATH}/parse-app/${this.slug}/envVars`,
          { withCredentials: true }
        )
      ).data;
    } catch (err) {
  ...
  async addCustomDomain(hostSettings) {
    try {
      return (
        await axios.post(
          // eslint-disable-next-line no-undef
  ...
  async getPushAndroidConfig() {
    try {
      return (
        await axios.get(
          // eslint-disable-next-line no-undef
          `${b4aSettings.BACK4APP_API_PATH}/parse-app/${this.slug}/push/android`,
          { withCredentials: true }
        )
      ).data;
    } catch (err) {
  ...
  async updatePushAndroidConfig(file) {
    try {
      const formData = new FormData();
      formData.append('firebaseServiceAccount', file);
      return (
        await axios.post(
          // eslint-disable-next-line no-undef
          `${b4aSettings.BACK4APP_API_PATH}/parse-app/${this.slug}/push/android`,
